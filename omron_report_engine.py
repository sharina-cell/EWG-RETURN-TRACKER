...
def reconcile_marketplace(
    ...
    for order_id in new_ids:
        ...
        if not sku:
            for col in ("SKU", "Seller SKU", "Seller SKU ID"):
                if col in group.columns:
                    vals = [str(s) for s in group[col].tolist() if pd.notna(s)]
                    sku = " / ".join(dict.fromkeys(vals)) if vals else None
                    break
        ...
        result.new_rows.append(NewReturnRow(
            ...
            invoice_date=_coerce_datetime(_safe_get(first, "Order Creation Date", "Order Date")),
            invoice_number=invoice if invoice and invoice != "nan" else None,
            tracking_number=_safe_get(first, "Return Tracking Number", "Return Logistic Tracking Number", "Return Logistics Tracking ID"),
            sku=sku,
            jde_model=None,   # filled from SKU->JDE map if available
