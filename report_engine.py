...
def read_tc_order_report(file_like) -> pd.DataFrame:
    ...
    Note on SKU columns (Omron-style TC exports): 'sku' here is TC's own
    internal item code (e.g. 'EWG0000038'), NOT the barcode the tracker's SKU
    column actually wants. The barcode lives in 'custom_sku' (e.g.
    4975479496288), and the human-readable model code the tracker calls
    "Product Model (JDE Model)" lives in a separate 'JDE code' column. Both
    are normalized here so downstream enrichment pulls the right values.
    """
    ...
    if "custom_sku" in df.columns:
        df["custom_sku"] = df["custom_sku"].apply(normalize_order_id)
    return df
...
def reconcile_marketplace(
    ...
    `marketplace` is the clean display name used in summaries/warnings (e.g.
    "Shopee"). `platform_label` is what actually gets written into the
    tracker's platform/marketplace column, which some brand trackers (e.g.
    Omron) format as "Shopee-Omron SG" rather than a bare "Shopee". Defaults
    to `marketplace` when not given.
    """
    result = ReconciliationResult(marketplace=marketplace)
    platform_label = platform_label or marketplace
    ...
    for order_id, group in grouped:
        ...
        if not tc_match.empty:
            ...
            # SKU should be the barcode ('custom_sku'), not TC's internal item
            # code ('sku') -- the tracker's SKU column expects barcodes like
            # 4975479496288, matching what marketplace exports show.
            custom_skus = [
                str(s) for s in tc_match.get("custom_sku", pd.Series(dtype=str)).tolist()
                if pd.notna(s) and str(s) != "nan"
            ]
            sku = " / ".join(dict.fromkeys(custom_skus)) if custom_skus else None
            jde_models = [
                str(s) for s in tc_match.get("JDE code", pd.Series(dtype=str)).tolist()
                if pd.notna(s) and str(s) != "nan"
            ]
            jde_model = " / ".join(dict.fromkeys(jde_models)) if jde_models else None
            notes = ""
        else:
...
# Note: "invoice date" intentionally maps to ordered_date as a fallback only for
# trackers that don't have a distinct ordered/order date column of their own --
# Omron's tracker has both "Invoice Date" and no separate "Order Date" column,
# and ops uses Invoice Date as the order-placed reference, so this fallback is
# correct for this template. Re-check this mapping if the same engine is later
# pointed at a tracker that has both columns simultaneously.
FIELD_HEADER_CANDIDATES: dict[str, tuple[str, ...]] = {
    ...
    "ordered_date": ("invoice date", "ordered date", "order date"),
    "invoice_number": ("invoice number", "invoice no"),
    "tracking_number": ("tracking number", "logistic tracking"),
    "sku": ("sku",),
    "jde_model": ("jde model", "product model", "jde code"),
    "qty": ("qty", "quantity"),
    "return_reason": ("return reason", "grass return reason"),
}
...
def run_reconciliation(
    ...
    `platform_labels` optionally maps the bare marketplace name ("Shopee",
    "Lazada", "TikTok") to the brand-specific string the tracker actually uses
    in its platform column (e.g. {"Shopee": "Shopee-Omron SG"}). Marketplaces
    not present in the map fall back to their bare name.
    """
    ...
    if shopee_file is not None:
        ...
        res = reconcile_marketplace(
            "Shopee", mp_df, tracker["existing_order_ids"], tc_df,
            platform_label=platform_labels.get("Shopee"),
        )
        results.append(res)
    ...
    if lazada_file is not None:
        ...
        res = reconcile_marketplace(
            "Lazada", mp_df, tracker["existing_order_ids"], tc_df,
            platform_label=platform_labels.get("Lazada"),
        )
        results.append(res)
        all_new_rows.extend(res.new_rows)
    ...
    if tiktok_file is not None:
        ...
        res = reconcile_marketplace(
            "TikTok", mp_df, tracker["existing_order_ids"], tc_df,
            platform_label=platform_labels.get("TikTok"),
        )
        results.append(res)
        all_new_rows.extend(res.new_rows)
